...
import { apiUrl, socketUrl } from 'src/app/services/serverUrl';
import { GeofenceComponent } from '../../dashboard/dashboard/geofence/geofence.component';
...
if (type == "circle") {
this.GeofenceseValue = ["CIRCLE ("+Object.values(layer._latlng).join(" ")+", "+layer._mRadius+")"]
localStorage.setItem("area",this.GeofenceseValue[0])
}else if (type == "polyline") {

 layer._latlngs.map(data => this.GeofenceseValue.push(Object.values(data).join(" ")))
 localStorage.setItem("area","POLYGON(("+this.GeofenceseValue.toString()+"))")

}
  // console.log(Object.values(layer._latlng).join(" "))
...
add() {
  ...
  showGeofenceOnMap(g:Geofence){
    ...
    if (g.area.includes("POLYGON")) {
      ...
      let circle = L.circle([centerPoint.lat, centerPoint.lng], Number(latLon[1]) * 1000);
